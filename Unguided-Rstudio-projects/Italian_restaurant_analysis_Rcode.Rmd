---
title: "Italian Resteurant analysis"
output:
...
* Dataset consists of 10000 observations and 9 variables namely : Date ,Time ,Menu item , Category , Quantity purchased ,Price per item , Revenue , Payment method , Customer type
...
data <-read_excel('Italian_resteurant_dataset.xlsx')

# Filtering data 

data$Date<-as.Date(data$Date)

data <-data %>%mutate(Morning_afternoon=str_extract(Time,'AM|PM'),Month=months.Date(Date),
                       Week_day=weekdays(Date))%>%select(-c(Date,Time))


mat <-matrix(0,nrow = 3,ncol = 2)
mat[,1]<-c('Price','Revenue','Quantity purchased')
mat[,2]<-c(mean(data$`Price (per item)`),mean(data$Revenue),mean(data$Quantity))
...
revenue_by_category <- data %>% select(Category,`Price (per item)`,Revenue,Quantity) %>%
  group_by(Category) %>% summarise(Average_price=mean(`Price (per item)`),Total_customers=length(Category),
  Total_quantity=sum(Quantity) ,Total_revenue=sum(Revenue)) %>% arrange(desc(Total_revenue))
...
Revenue_per_category_by_menu_item <- data %>% select(Category,Quantity,`Menu Item`,Revenue)%>%
  group_by(Category,`Menu Item`) %>% summarise(Total_customers=length(`Menu Item`),                    Total_quantity=sum(Quantity),Total_Revenue=sum(Revenue))
...
customer_preferred_time <- data%>% select(Morning_afternoon,Revenue) %>%
  group_by(Morning_afternoon)%>%
summarise(Total_customers=length(Morning_afternoon),Revenue=sum(Revenue))

kable(customer_preferred_time)

customer_preferred_time %>% ggplot(aes(x=Morning_afternoon,y=Revenue,fill = Morning_afternoon)) +
...
customer_type%>%ggplot(aes(x=`Customer Type`,y=Total_customers)) +
...
weekly_revenue <- data %>% select(Week_day,Revenue) %>% group_by(Week_day) %>%
  summarise(Total_revenue=sum(Revenue)) %>% arrange(factor(Week_day,
