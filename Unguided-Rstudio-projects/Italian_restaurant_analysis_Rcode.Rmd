...
data <-read_excel('C:\\Users\\karab\\Videos\\JOB_Projects\\Italian_resteurant_dataset.xlsx')
head(data)
...
data %>%ggplot(aes(x=`Price (per item)`,y=Revenue))+
geom_point(,size=2.5)+labs(x='Price',title='Price_Revenue relationship',y='Revenu (Euro)')+
...
customer_preferred_time <- data%>% select(AM_PM,Revenue) %>%
  group_by(AM_PM)%>%
summarise(Total_customers=length(AM_PM),Revenue=sum(Revenue))
